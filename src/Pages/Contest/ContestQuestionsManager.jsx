...
export default function ContestQuestionsManager() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setContest(c.data?.contest || c.contest || c.data);
  ...
  const onCreate = async (e) => {
    ...
    try {
      ...
      if (editor._id) {
        ...
        const updatedQuestion = res.data?.question || res.question || res.data;
        setQuestions(qs => qs.map(q => q._id === editor._id ? updatedQuestion : q));
      } else {
        const res = await createContestQuestion(contestId, payload);
        const newQuestion = res.data?.question || res.question || res.data;
        setQuestions(qs => [...qs, newQuestion].sort((a,b) => a.order - b.order));
      }
  ...
  const TypeFields = useMemo(() => {
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <button type="button" className="px-3 py-1 rounded border text-sm" 
            onClick={() => setEditor(ed => ({ ...ed, examples: [...(ed.examples || []), { input: '', output: '', explanation: '' }] }))}>
            + Add Example
          </button>
        </div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setEditor((ed) => ({
                ...ed,
                starterCode: {
                  ...normalizeStarterCode(ed.starterCode),
              ...
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <label className="flex items-center gap-1 text-sm">
                    <input type="checkbox" checked={t.isPublic !== false} 
                      onChange={(e) => {
    ...
    return null;
  }, [editor, starterLang, availableLanguages, starterCodeObj]);
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button className="px-2 py-1 rounded border" onClick={() => {
                      ...
                      const mergedStarterCode = type === 'CODING'
                        ? { ...defaults.starterCode, ...normalizeStarterCode(q.starterCode) }
                        : (q.starterCode || defaults.starterCode || {});
                      setEditor({
                        ...defaults,
                        ...q,
                        type,
                        ...
                        examples: q.examples?.length ? q.examples : (defaults.examples || [{ input: '', output: '', explanation: '' }]),
                      ...
                      if (type === 'CODING') {
                        const keys = Object.keys(mergedStarterCode || {});
                        const next = keys.includes('javascript') ? 'javascript' : (keys[0] || 'javascript');
                        setStarterLang(next);
                      }
                    }}>Edit</button>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setEditor((prev) => {
                      ...
                      // Preserve common fields while applying type-specific defaults
                      return {
                        ...next,
                        _id: prev._id,
                        title: prev.title,
